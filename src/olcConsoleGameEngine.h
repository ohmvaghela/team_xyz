...
{
    FILE *f = nullptr;
    f = std::fopen(std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(sFile).c_str(), "wb");
    if (f == nullptr)
        return false;
    ...
    std::fwrite(m_Glyphs, sizeof(short), nWidth * nHeight, f);
...
bool Load(std::wstring sFile)
{
    ...
    FILE *f = nullptr;
    f = std::fopen(std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(sFile).c_str(), "rb");
    if (f == nullptr)
        return false;
...
olcConsoleGameEngine()
{
    ...
    m_nScreenWidth = ws.ws_col;
    m_nScreenHeight = ws.ws_row;
